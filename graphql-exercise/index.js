...
const resolvers = {
  ...
  Mutation:{
    addBook:(root,args) => {
        ...
        if(!author){
            const newAuthor = {name:args.author,id:randomUUID()}
            authors = authors.concat(newAuthor)
        }
        return book
    },
    addAuthor:(root,args) => {
        const author = {...args,id: randomUUID()}
        authors = authors.concat(author)
        return author
    },
    editAuthor:(root,args) => {
        const newAuthor = {name: args.name, born: args.setBornTo}
        authors = authors.map(author => author.name==args.name ? {...author,name:newAuthor.name,born:newAuthor.born}: author)
